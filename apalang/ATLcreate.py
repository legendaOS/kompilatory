...
def Shstr(BufferRoot):
    """
    Обход в обратную ширину
    """
    allNodes = Rstr(BufferRoot)
    ret = []

    while True:
        if len(allNodes) == 0: break
        for ind in range(len(allNodes)):
            noda = allNodes[ind]
            if len(noda.Childrens) == 0 or all([(i in ret) for i in noda.Childrens]):
                ret.append(noda)
                allNodes.pop(ind)
                break
